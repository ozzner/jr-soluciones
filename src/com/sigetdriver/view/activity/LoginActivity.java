...
public class LoginActivity extends Activity {
	...
	class loginValidar extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			respuesta = SigetDriverUtil.connect(URL);
			...
			try {
				...
				Log.e("RESLTADO RESPUESTA", respuesta);
		...
		protected void onPostExecute(Void result) {
			...
			if (login) {
				...
				ServicioWorkingSet.usuarioLogin = edtUsuario.getText()
						.toString().trim();
				ServicioWorkingSet.passwordLogin = SigetDriverUtil
						.encryptPassword(edtPassword.getText().toString()
								.trim());
				...
				Toast.makeText(getApplicationContext(),
						"Verifique su conexión a Internet", Toast.LENGTH_SHORT)
						.show();
			}
	...
	class preSincronizarDatos extends AsyncTask<String, Void, Void> {
		...
		protected Void doInBackground(String... params) {
			...
			String URL = ServerConstants.SERVER_IP
					+ ServerConstants.PRESINCRONIZACION + params[0] + ","
					+ params[1] + "/" + params[3];
			...
			try {
				...
				while (jParser.nextToken() != JsonToken.END_OBJECT) {
					...
					if ("tarifasZonas".equals(fieldname)) {
						jParser.nextToken();
						contadorMaestro += jParser.getIntValue();
						System.out.println("TARIFAS ZONAS = "
								+ jParser.getIntValue());
					}
					// [CENTROS COSTOS]
					if ("centroCostos".equals(fieldname)) {
						jParser.nextToken();
						contadorMaestro += jParser.getIntValue();
						System.out.println("CENTROS COSTOS = "
								+ jParser.getIntValue());
					}
				}
		...
		protected void onPostExecute(Void result) {
			...
			if (contadorMaestro != 0) {
				...
				Toast.makeText(getApplicationContext(), "Datos Actualizados",
						Toast.LENGTH_SHORT).show();
				Toast.makeText(getApplicationContext(),
						"Bienvenido: " + ServicioWorkingSet.nombreConductor,
						Toast.LENGTH_SHORT).show();
				Intent i = new Intent(LoginActivity.this, EstadoActivity.class);
				Log.e("CHECK", checkname);
				Log.e("REG ID", regId);
				i.putExtra("CHECK", checkname);
				startActivity(i);
				finish();
			}
	...
	class sincronizarDatos extends AsyncTask<String, Integer, Integer> {
		...
		protected Integer doInBackground(String... params) {
			...
			String URL = ServerConstants.SERVER_IP
					+ ServerConstants.SINCRONIZACION + params[0] + ","
					+ params[1] + "/" + params[3];
			...
			try {
				...
				while (jParser.nextToken() != JsonToken.END_OBJECT) {
					...
					if ("empresas".equals(fieldname)) {
						...
						while (jParser.nextToken() != JsonToken.END_ARRAY) {
							if ("codEmpresa".equals(jParser.getCurrentName())) {
								...
								jParser.nextToken();
								empresa.setMontoHoraInterurbana(jParser
										.getText());
								jParser.nextToken();
								// 12-montoDiaUrbano
								jParser.nextToken();
								...
								jParser.nextToken();
								empresa.setMontoDiaInterurbano(jParser
										.getText());
								jParser.nextToken();
								// 14-montoCancelacionUrbana
								jParser.nextToken();
								empresa.setMontoCancelacionUrbana(jParser
										.getText());
								jParser.nextToken();
								// 15-montoCancelacionInterurbana
								jParser.nextToken();
								empresa.setMontoCancelacionInterurbana(jParser
										.getText());
								jParser.nextToken();
								// 16-montoCancelacionAeropuerto
								jParser.nextToken();
								empresa.setMontoCancelacionAeropuerto(jParser
										.getText());
								jParser.nextToken();
								// 17-minTiempoEspera1
								jParser.nextToken();
								...
								jParser.nextToken();
								empresa.setMinTiempoEsperaRecurrente(jParser
										.getText());
								jParser.nextToken();
								...
								jParser.nextToken();
								empresa.setMontoTiempoEsperaRecurrente(jParser
										.getText());
								jParser.nextToken();
								// 25-sobrecargoDesvio
								jParser.nextToken();
					...
					if ("pasajeros".equals(fieldname)) {
						...
						while (jParser.nextToken() != JsonToken.END_ARRAY) {
							if ("codigo".equals(jParser.getCurrentName())) {
								...
								PasajeroController.getInstance()
					...
					if ("zonas".equals(fieldname)) {
						...
						while (jParser.nextToken() != JsonToken.END_ARRAY) {
							if ("id".equals(jParser.getCurrentName())) {
								...
								ZonaController.getInstance().actualizarZona(
										zona);
					...
					if ("sedes".equals(fieldname)) {
						...
						while (jParser.nextToken() != JsonToken.END_ARRAY) {
							if ("id".equals(jParser.getCurrentName())) {
								...
								SedeController.getInstance().actualizarSede(
										sede);
					...
					if ("tarifasZonas".equals(fieldname)) {
						...
						while (jParser.nextToken() != JsonToken.END_ARRAY) {
							if ("id".equals(jParser.getCurrentName())) {
								...
								TarifaZonaController.getInstance()
										.actualizarTarifaZona(tarifaZona);
					...
					if ("centroCostos".equals(fieldname)) {
						...
						while (jParser.nextToken() != JsonToken.END_ARRAY) {
							if ("id".equals(jParser.getCurrentName())) {
								...
								CentroCostoController.getInstance()
										.actualizarCentroCosto(centroCosto);
					...
					if ("fechaSincronizacion".equals(fieldname)) {
						...
						System.out.println("FECHA SINCRONIZACION: "
								+ fechaSincronizacion);
			...
			System.out
					.println("[CENTROS COSTOS] = " + listaCentroCostos.size());
			System.out.println("-------------------------------\n");

			System.out.println("---SINCRONIZACION SQLITE---");
			System.out.println("[EMPRESAS] = "
					+ EmpresaBean.tableHelper.countAllRows());
			System.out.println("[PASAJEROS] = "
					+ PasajeroBean.tableHelper.countAllRows());
			System.out.println("[ZONAS] = "
					+ ZonaBean.tableHelper.countAllRows());
			System.out.println("[SEDES] = "
					+ SedeBean.tableHelper.countAllRows());
			System.out.println("[TARIFAS ZONAS] = "
					+ TarifaZonaBean.tableHelper.countAllRows());
			System.out.println("[CENTROS COSTOS] = "
					+ CentroCostoBean.tableHelper.countAllRows());
			System.out.println("-------------------------------\n");
