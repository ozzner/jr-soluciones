...
public class ServicioController {
	...
	static ArrayList<ServicioBean> listaServicios =
			new ArrayList<ServicioBean>();
	...
	public ArrayList<ServicioBean> obtenerServiciosPendientes() {
		...
		ArrayList<Entity> entities = ServicioBean.tableHelper.getEntities(
				 ServicioBean.ESTADO_COLUMN_NAME + "=? OR " +
						 ServicioBean.ESTADO_COLUMN_NAME + "=?",
				 new String[] { ServicioBean.ESTADO_PENDIENTE, ServicioBean.ESTADO_ACTIVO });
		...
		for (int i=(len-1) ; i>0 ; i--) {
			for (int j=0 ; j<i ; j++) {
				if (Long.parseLong(listaServicios.get(j).getFecha()) >
					Long.parseLong(listaServicios.get(j+1).getFecha()) ) {
	...
	static ArrayList<PuntoBean> listaPuntos =
			new ArrayList<PuntoBean>();
	...
	public void descargarServicio(String id) {
		...
		try {
			...
			servicio.setIdServicio(
					jsonPadre.getString("numReserva"));
...
//			DateFormat formatHora = new SimpleDateFormat("HH:mm");
			...
			servicio.setEmpresa(
					jsonPadre.getString("empresaRazonSocial"));			
			servicio.setPasajero(
					jsonPadre.getString("pasajeroNombres") + " " + 
					jsonPadre.getString("pasajeroApellidos"));
			servicio.setCelular(
					jsonPadre.getString("pasajeroTelefonos"));
			servicio.setObservaciones(
					jsonPadre.getString("observacion"));
			servicio.setCodigo(
					jsonPadre.getString("codigoVerificacion"));
			...
			puntoOrigen.setIdServicio(
					jsonPadre.getString("numReserva"));
			puntoOrigen.setLatitudPunto(
					jsonPadre.getString("origenLatitud"));
			puntoOrigen.setLongitudPunto(
					jsonPadre.getString("origenLongitud"));
			puntoOrigen.setDireccion(
					jsonPadre.getString("origenDireccion"));
			puntoOrigen.setZona(
					jsonPadre.getString("origenZonaNombre"));
			puntoOrigen.setOrden(
			...
			for (int i=0 ; i<destinos.length() ; i++) {
				...
				puntoDestino.setIdServicio(
						jsonPadre.getString("numReserva"));
				puntoDestino.setLatitudPunto(
						destino.getString("puntoLatitud"));
				puntoDestino.setLongitudPunto(
						destino.getString("puntoLongitud"));
				puntoDestino.setDireccion(
						destino.getString("puntoDireccion"));
				puntoDestino.setZona(
						destino.getString("puntoZonaNombre"));
				puntoDestino.setOrden(
						String.valueOf(orden));
				orden = orden + 2;
